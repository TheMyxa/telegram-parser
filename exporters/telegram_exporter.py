...
def configure(export_config, postgres_config=None):
    global API_ID
    global API_HASH
    global CHANNELS
    ...
    global OUTPUT_DIR
    global POST_LIMIT
    global INCREMENTAL_LOOKBACK_POSTS
    global POSTS_PAUSE_SECONDS
    global POSTS_PAUSE_AFTER_POSTS
    global POSTGRES_HOST
    global POSTGRES_PORT
    global POSTGRES_DB
    global POSTGRES_USER
    global POSTGRES_PASSWORD
    global POSTGRES_TABLE

    API_ID = export_config.api_id
    API_HASH = export_config.api_hash
    CHANNELS = export_config.channels
    TELEGRAM_SESSION = export_config.telegram_session
    OUTPUT_DIR = os.path.dirname(export_config.output_file) or "."
    POST_LIMIT = export_config.post_limit
    INCREMENTAL_LOOKBACK_POSTS = export_config.incremental_lookback_posts
    POSTS_PAUSE_SECONDS = export_config.posts_pause_seconds
    POSTS_PAUSE_AFTER_POSTS = export_config.posts_pause_after_posts
    ...
    if postgres_config:
        ...
        POSTGRES_TABLE = postgres_config.table
...
def save_json(export_data, path):
    write_json_atomic(export_data, path, indent=5)
...
async def main(args=None):
    ...
    postgres_config = load_postgres_config() if args.export_format == "postgresql" else None
    configure(export_config, postgres_config)
